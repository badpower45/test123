...
class BreakRequestsTab extends StatefulWidget {
  ...
  @override
  State<BreakRequestsTab> createState() => _BreakRequestsTabState();
}

class _BreakRequestsTabState extends State<BreakRequestsTab> {
  ...
  @override
  void didUpdateWidget(covariant BreakRequestsTab oldWidget) {
  ...
  Future<void> _loadBreaks({bool showLoadingIndicator = true}) async {
    ...
    try {
      final breaks = await RequestsApiService.fetchBreaks(employeeId: widget.employeeId);
      if (!mounted) {
        return;
      }
      setState(() {
  ...
  Future<void> _openBreakRequestSheet() async {
    final hasShift = await RequestsApiService.checkActiveShift(widget.employeeId);
    if (!hasShift) {
      if (mounted) {
    ...
    final result = await showModalBottomSheet<bool>(
      ...
      builder: (context) => _BreakRequestSheet(employeeId: widget.employeeId),
    );
