...
import { pulses, branchEnvironmentBaselines, } from '../../shared/schema.js';
import { eq, sql, and, isNotNull } from 'drizzle-orm';
import { sendAlert } from './notification-service.js';
export async function detectBaselineDrift(branchId, timeSlot) {
    ...
    const recentPulses = await db
        ...
        .from(pulses)
        .where(and(eq(pulses.branchId, branchId), isNotNull(pulses.wifiCount)))
    ...
    const driftDetected = wifiDrift > WIFI_THRESHOLD ||
 ...
 */
export async function detectAllBranchDrift() {
