...
import { branchEnvironmentBaselines, deviceCalibrations, employeeDeviceBaselines, blvSystemConfig, pulseFlags } from '../../shared/schema.js';
 ...
 */
function normalizeReading(value, calibrationFactor, readingType) {
    return value * calibrationFactor;
}
/**
 * Calculate similarity score between two values with tolerance
 */
function calculateSimilarityScore(actual, expected, stdDev, tolerance = 2.0) {
 ...
 */
function calculateRangeScore(actual, min, max, tolerance = 0.2) {
 ...
 */
export async function verifyPresence(employeeId, branchId, environmentalData, bssidAddress) {
    try {
        // 1. Get configuration
        ...
        const presenceScore = (wifiScore * config.wifiWeight +
...
// ============================================================================
function calculateWifiScore(data, baseline, config) {
    if (!baseline.avgWifiCount)
        return 0.5; // No baseline data
    // Score based on WiFi count similarity
    const countScore = calculateSimilarityScore(data.wifiCount, baseline.avgWifiCount, baseline.wifiCountStdDev || 2, 2.0);
    ...
    if (data.wifiSignalStrength && baseline.avgSignalStrength) {
        signalScore = calculateSimilarityScore(data.wifiSignalStrength, baseline.avgSignalStrength, 10, // ±10 dBm tolerance
        2.0);
    }
    // Weighted combination
    return countScore * 0.7 + signalScore * 0.3;
}
function calculateMotionScore(data, baseline, employeeBaseline, config) {
    ...
    return calculateSimilarityScore(data.accelVariance, expectedVariance, stdDev, 2.5 // More tolerance for motion
    );
}
function calculateSoundScore(data, baseline, config) {
    if (!baseline.avgSoundLevel)
        return 0.5; // No baseline
    return calculateSimilarityScore(data.soundLevel, baseline.avgSoundLevel, baseline.soundLevelStdDev || 0.1, 2.0);
}
function calculateBatteryScore(data, baseline, config) {
    if (!baseline.avgBatteryLevel)
        return 0.5; // No baseline
    // Battery level score
    const levelScore = calculateSimilarityScore(data.batteryLevel, baseline.avgBatteryLevel, 0.2, // ±20% tolerance
    2.0);
...
// ============================================================================
function calculateTrustScore(data, baseline, employeeBaseline) {
    let trustScore = 1.0;
    const flags = [];
...
async function getConfig(branchId) {
    ...
    const branchConfig = await db
        ...
        .where(and(eq(blvSystemConfig.branchId, branchId), eq(blvSystemConfig.isActive, true)))
        .limit(1);
    ...
    const globalConfig = await db
        .select()
        .from(blvSystemConfig)
        .where(and(eq(blvSystemConfig.branchId, null), eq(blvSystemConfig.isActive, true)))
        .limit(1);
...
async function getDeviceCalibration(deviceModel) {
...
function normalizeEnvironmentalData(data, calibration) {
    return {
        ...data,
...
async function getBranchBaseline(branchId, timeSlot) {
    const baselines = await db
        .select()
        .from(branchEnvironmentBaselines)
        .where(and(eq(branchEnvironmentBaselines.branchId, branchId), eq(branchEnvironmentBaselines.timeSlot, timeSlot)))
        .limit(1);
...
async function getEmployeeBaseline(employeeId, deviceModel) {
    const baselines = await db
        .select()
        .from(employeeDeviceBaselines)
        .where(and(eq(employeeDeviceBaselines.employeeId, employeeId), eq(employeeDeviceBaselines.deviceModel, deviceModel)))
        .limit(1);
    if (baselines.length > 0) {
        return baselines[0];
    }
...
// AUTO-FLAG CREATION
// ============================================================================
export async function createAutoFlags(pulseId, employeeId, flags, verificationResult) {
