...
import { pulses, branchEnvironmentBaselines, employeeDeviceBaselines } from '../../shared/schema.js';
 ...
 */
export function calculateBaselineConfidence(baselineConfidence, lastUpdated, currentDate = new Date()) {
 ...
 */
export function shouldRecalibrateBaseline(baselineConfidence, lastUpdated, threshold = 0.5) {
    const currentConfidence = calculateBaselineConfidence(baselineConfidence, lastUpdated);
 ...
 */
export async function calculateBranchBaseline(branchId, timeSlot, startDate, endDate) {
    ...
    const pulsesData = await db
        ...
        .from(pulses)
        .where(and(eq(pulses.branchId, branchId), gte(pulses.timestamp, startDate), lte(pulses.timestamp, endDate), isNotNull(pulses.wifiCount), isNotNull(pulses.accelVariance)));
    // Filter by time slot
 ...
 */
export async function calculateAllBaselines(branchId, daysBack = 14) {
 ...
 */
export async function calculateEmployeeBaseline(employeeId, deviceId, deviceModel, daysBack = 30) {
    ...
    const pulsesData = await db
        ...
        .from(pulses)
        .where(and(eq(pulses.employeeId, employeeId), gte(pulses.timestamp, startDate), lte(pulses.timestamp, endDate), isNotNull(pulses.accelVariance), eq(pulses.deviceModel, deviceModel)));
    ...
    const existing = await db
        ...
        .where(and(eq(employeeDeviceBaselines.employeeId, employeeId), eq(employeeDeviceBaselines.deviceId, deviceId)))
        .limit(1);
    if (existing.length > 0) {
        // Update
        await db
...
// STATISTICS HELPERS
// ============================================================================
function calculateStatistics(pulses) {
    return {
...
function blend(oldValue, newValue, alpha) {
    if (oldValue === null)
        return newValue;
    if (newValue === null)
        return oldValue;
 ...
 */
export async function updateAllBranchBaselines() {
