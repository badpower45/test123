    ...
    try {
        ...
        if (activeBreak) {
            ...
            const result = await db
                ...
                .from(pulses)
                .where(and(eq(pulses.employeeId, employeeId), gte(pulses.createdAt, startTs), lte(pulses.createdAt, now)));
            ...
            const result = await db
                .select({ count: sql `count(*)` })
                .from(pulses)
                .where(and(eq(pulses.employeeId, employeeId), eq(pulses.isWithinGeofence, true), // This is set to true if (WiFi OR Location)
            gte(pulses.createdAt, startTs), lte(pulses.createdAt, now)));
            validPulseCount = Number(result[0]?.count) || 0;
    ...
    try {
        ...
        if (start_date && end_date) {
            ...
            attendanceRecords = await db
                ...
                .from(attendance)
                .innerJoin(employees, eq(attendance.employeeId, employees.id))
                .where(and(gte(attendance.date, start_date), lte(attendance.date, end_date), branch_id ? eq(employees.branchId, branch_id) : sql `true`))
                .orderBy(attendance.date, employees.fullName);
        }
        else {
            // Single day export
            const targetDate = date ? date : getTodayEgypt();
...
app.get('/api/owner/employee-attendance/:employeeId', async (req, res) => {
    try {
        ...
        const approvedLeaves = await db
            .select()
            .from(leaveRequests)
            .where(and(eq(leaveRequests.employeeId, employeeId), eq(leaveRequests.status, 'approved'), gte(leaveRequests.startDate, startDate), lte(leaveRequests.endDate, endDate)));
