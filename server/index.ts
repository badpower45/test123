...
import {
  ...
  branchEnvironmentBaselines, pulseFlags, manualOverrides
...
app.post('/api/attendance/check-in', async (req, res) => {
  try {
    ...
    if (wifi_count !== undefined && accel_variance !== undefined && employee.branchId) {
      try {
        const blvResult = await verifyPresence(
          employee_id,
          employee.branchId,
          {
    ...
    if (employee && employee.role === 'manager' && request) {
      ...
      if (ownerId) {
        await sendNotification(
          ownerId,
          'ATTENDANCE_REQUEST',
          'طلب تصحيح حضور من مدير',
          `${employee.fullName} يطلب تصحيح وقت الحضور`,
          employee_id,
          request.id || ''
        );
    ...
    if (employee && employee.role === 'manager' && request) {
      ...
      if (ownerId) {
        await sendNotification(
          ownerId,
          'ATTENDANCE_REQUEST',
          'طلب تصحيح انصراف من مدير',
          `${employee.fullName} يطلب تصحيح وقت الانصراف`,
          employee_id,
          request.id || ''
        );
...
app.post('/api/pulses', async (req, res) => {
  try {
    ...
    if (wifi_count !== undefined && accel_variance !== undefined && employee.branchId) {
      try {
        blvResult = await verifyPresence(
          employee_id,
          employee.branchId,
          {
            wifiCount: wifi_count || 0,
  ...
  try {
    ...
    if (action === 'approve') {
      await sendNotification(
        ...
        'BREAK_APPROVED',
        'تمت الموافقة على طلب الاستراحة',
        `تمت الموافقة على طلب الاستراحة الخاص بك لمدة ${breakRecord.requestedDurationMinutes} دقيقة`,
        manager_id,
        breakId
      );
    } else if (action === 'reject') {
      await sendNotification(
        breakRecord.employeeId,
        'BREAK_REJECTED',
        'تم رفض طلب الاستراحة',
        `تم رفض طلب الاستراحة الخاص بك لمدة ${breakRecord.requestedDurationMinutes} دقيقة`,
        manager_id,
        breakId
      );
    }
...
cron.schedule('*/30 * * * *', async () => {
  try {
    ...
    for (const employee of allEmployees) {
      if (!employee.shiftStartTime || !employee.shiftEndTime) continue;
      ...
      const [existingAbsenceNotif] = await db
        ...
        .from(absenceNotifications)
        .where(and(
          eq(absenceNotifications.employeeId, employee.id),
          eq(absenceNotifications.absenceDate, today)
        ))
        .limit(1);

      if (existingAbsenceNotif) continue; // Already created absence notification
      ...
      if (notificationRecipientId) {
        ...
        await sendNotification(
          ...
          'ABSENCE_ALERT',
          `غياب ${roleLabel}`,
          `تنبيه: ${roleLabel} ${employee.fullName} لم يسجل حضوره بعد ساعتين من بداية الشيفت (${employee.shiftStartTime}). الخصم المقترح: ${twoDayDeduction.toFixed(2)} جنيه (يومين). يرجى الموافقة أو الرفض.`,
          employee.id,
          newAbsenceNotif.id
        );
...
app.get('/api/flags', async (req, res) => {
  try {
    ...
    if (branchId) {
      query = query.where(eq(employees.branchId, branchId as string));
    }

    if (severity) {
      query = query.where(eq(pulseFlags.severity, severity as string));
    }
...
app.post('/api/overrides', async (req, res) => {
  try {
    ...
    const pulse = extractFirstRow(originalPulse) as any;
